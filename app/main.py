...
from . import models
from . import schemas
from .auth import hash_password, verify_password, create_access_token, verify_google_id_token
...
def register_start(payload: schemas.RegisterStartIn, db: Session = Depends(get_db)):
    ...
    if not user:
        ...
        db.add(user)
        db.flush()
    # Update password hash (even if existing but unverified)
    ...
    v = models.EmailVerificationCode(user_id=user.id, code_hash=code_hash, expires_at=expires_at)
    db.add(v)
    db.commit()
...
def google_sign_in(payload: schemas.GoogleSignInIn, db: Session = Depends(get_db)):
    ...
    if not user:
        # Create new user
        user = models.User(email=email or f"google_{sub}@google.local", google_sub=sub, is_verified=True)
        db.add(user)
    else:
        user.google_sub = sub
        user.is_verified = True
...
def telegram_sign_in(payload: schemas.TelegramSignInIn, db: Session = Depends(get_db)):
    ...
    if not user:
        # Ensure unique email even without real email
        synthetic_email = f"tg_{tg_id}@telegram.local"
        existing = db.query(models.User).filter(models.User.email == synthetic_email).first()
        if existing:
            synthetic_email = f"tg_{tg_id}_{int(datetime.utcnow().timestamp())}@telegram.local"
        user = models.User(email=synthetic_email, telegram_id=tg_id, is_verified=True)
        db.add(user)
        db.commit()
    token = create_access_token(user.id)
    return schemas.Token(access_token=token)
